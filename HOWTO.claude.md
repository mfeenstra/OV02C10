...
(A point-in-time snapshot — expect the exact `-archN` suffix and DKMS module
path below to drift with routine `pacman -Syu` updates; run `uname -r` and
`modinfo ov02c10 | grep filename` to check yours. This has since moved on to
`7.1.3-arch1-3` with no change in camera behavior.)
...
Also in this repo as [`pkglist.txt`](pkglist.txt).

> **Note on `libcamera-ipu6`:** its AUR `PKGBUILD` has a bug — it references
> `../` paths assuming the default build directory, which breaks if you have
> a custom `BUILDDIR` set. A future `yay -Syu` may hit this again if it pulls
> an updated `PKGBUILD` from the AUR, overwriting any local fix and failing
> the same way until the maintainer fixes it upstream. Worth leaving a
> comment on the `libcamera-ipu6` AUR page asking them to change the `../`
...
### Module load order (why the CVS handshake above just works on boot)

The probe-fails-then-succeeds dance above is expected — it relies on the
kernel modules involved loading in a specific order every boot, driven by
three `/etc/modules-load.d/` files:

- [`camera.conf`](camera.conf) — `intel_skl_int3472_discrete`, `intel_cvs`,
  `ov02c10`
- [`intel-ipu6.conf`](intel-ipu6.conf) — `intel_ipu6`, `intel_ipu6_isys`
- [`modules-load.d-v4l2loopback.conf`](modules-load.d-v4l2loopback.conf) —
  `v4l2loopback` (renamed in this repo to avoid colliding with the
  `modprobe.d` file of the same basename in Step 8)

The actual load-before/load-after ordering between these comes from
`softdep` directives in [`ipu6-camera.conf`](ipu6-camera.conf) under
`/etc/modprobe.d/`: `intel_cvs` before `intel_ipu6`, before
`intel_ipu6_isys`, before `ov02c10`, before `v4l2loopback`.

This ordering matters beyond just Step 1: it's also why a plain reboot is
the reliable fix any time WirePlumber and the camera kernel modules get out
of sync (see the "stuck PipeWire v4l2 node" entry in Troubleshooting) —
on a normal boot, all of the above finishes loading well before WirePlumber
ever starts, so there's no race between them.

There's also `/etc/tmpfiles.d/`[`intel-camera.conf`](intel-camera.conf),
which creates `/run/camera` at boot for the HAL's AIQ runtime data. On this
system it currently logs a harmless-but-real error every boot
(`Failed to resolve group 'camera': Unknown group` from
`systemd-tmpfiles-setup.service`, since no such group exists) — `run-webcam.sh`
papers over this by creating the directory itself if missing, but the
proper fix is either creating a `camera` group or changing the rule to use
an existing one.
...
- The camera driver's shared-memory mechanism normally requires root permissions, so the systemd service needs a specific setting relaxed (see step 2 below) to allow it.
...
- Create/edit: `/etc/v4l2-relayd.d/intel-ipu.conf`
  (also in this repo as [`intel-ipu.conf`](intel-ipu.conf)) with:
...
- Create/edit: `/etc/default/v4l2-relayd`
  (also in this repo as [`v4l2-relayd`](v4l2-relayd))
...
> **Precedence note:** the packaged unit loads `/etc/default/v4l2-relayd`
> first, then `/etc/v4l2-relayd.d/%i.conf` (i.e. `intel-ipu.conf` for this
> `@intel-ipu` instance) — see `systemctl cat v4l2-relayd@intel-ipu.service`.
> The per-instance file wins on any variable both define. In practice this
> means `intel-ipu.conf`'s `FORMAT` is what actually reaches the pipeline
> even if `/etc/default/v4l2-relayd` disagrees — worth knowing so a stale
> value in the latter doesn't cost you a debugging session.
...
- Create/edit: `/etc/systemd/system/v4l2-relayd@intel-ipu.service.d/icamerasrc.conf`
  (also in this repo as [`icamerasrc.conf`](icamerasrc.conf))
...
2b. **Use a v4l2-relayd that understands v4l2loopback ≥ 0.13.**
...
- Create `/etc/wireplumber/wireplumber.conf.d/50-hide-ipu6-isys.conf`
  (also in this repo as [`50-hide-ipu6-isys.conf`](50-hide-ipu6-isys.conf)):
...
- Set `exclusive_caps=1` in `/etc/modprobe.d/v4l2loopback.conf`
  (also in this repo as [`modprobe.d-v4l2loopback.conf`](modprobe.d-v4l2loopback.conf) —
  renamed to avoid colliding with the `modules-load.d` file of the same
  basename, see below) so apps that probe V4L2 directly (Chrome, Zoom,
  Discord) treat the device as a real capture-only webcam:
...
```
# Hide the raw Intel IPU6 ISYS capture nodes (/dev/video0-47) from regular
# users. Only the root-owned v4l2-relayd/CamHAL pipeline needs them; user apps
# (Chromium/Electron: Hyperbeam, Discord, browsers) otherwise enumerate 48
# dead cameras and usually default to the first one.
# Must sort after 70-uaccess.rules (which adds TAG "uaccess") and before
# 73-seat-late.rules (which turns that tag into a per-user ACL).
...
components. Double-check `VIDEOSRC` in Step 6 matches exactly. If the
...
root half of Step 4. Another is `FRAMERATE=30` instead of `FRAMERATE=30/1`
...
**PipeWire apps (Cheese, browsers, etc.) fail or hang, but `qv4l2`/`ffmpeg` work fine**
Check `journalctl --user -u wireplumber` for:
```
pipewire[...]: spa.v4l2: '/dev/videoNN' VIDIOC_S_FMT: Device or resource busy
```
WirePlumber's PipeWire v4l2 source node for the virtual webcam got stuck in
an already-streaming state (usually since boot). Every new client that links
to it makes PipeWire re-issue `VIDIOC_S_FMT` on that handle — even to the
exact format already active — and the kernel correctly refuses to reformat a
device mid-stream. `qv4l2` and `ffmpeg` don't hit this because they open
their own fresh handle instead of reusing the stuck node.

Fix it with `fix-stuck-webcam-node.sh` in this repo, which destroys the
stuck PipeWire node and cycles a udev remove/add event (a plain `change`
event isn't enough) to make WirePlumber recreate it fresh:
```bash
./fix-stuck-webcam-node.sh            # defaults to /dev/video48
```
...
rule from Step 2 is meant to fix this automatically, but you can also force
it by hand:
```bash
...
the on-demand one from Step 6. Switch back with:
```bash
...
If the on-demand setup from Step 6 turns out to be unreliable on your
...
## Appendix — Which camera app to use (and why some conflict)

Not every video app that can open a camera will work here — which ones do
depends on *how* they get frames. There are three distinct paths into the
pipeline, and they don't all coexist:

- **(a) PipeWire/portal path** — the app asks the xdg camera portal, which
  hands it a PipeWire stream from WirePlumber's node for `Virtual_WebCam_0`.
  This is the intended path (see the architecture diagram at the top of
  `CLAUDE.md`).
- **(b) Direct V4L2 path** — the app opens `/dev/video48` itself, bypassing
  PipeWire. Whether this works depends on whether it collides with
  PipeWire's own open handle/buffer allocation on that device.
- **(c) Direct libcamera path** — the app talks to `libcamera`/the OV02C10
  sensor directly, bypassing `/dev/video48` and the relay entirely. This
  conflicts with the relay's own exclusive libcamera acquisition
  (`icamerasrc`, inside `v4l2-relayd@intel-ipu.service`) — only one
  libcamera client can hold a camera at a time.

| App(s) | Path | Result | Why |
|---|---|---|---|
| Cheese, GNOME Camera/Snapshot, browsers (Chrome/Firefox WebRTC), Zoom, Discord, OBS (PipeWire Camera source) | (a) portal → PipeWire | **Works** | Intended path — this is the target app list per `CLAUDE.md`'s architecture section |
...
| `guvcview` | (b) direct V4L2, `mmap` capture | **Fails** — `VIDIOC_REQBUFS`/`VIDIOC_S_FORMAT: Device or resource busy` | PipeWire already holds the loopback's one mmap buffer allocation (`v4l2loopback` here is `max_buffers: 2`, no room for a second independent mmap owner). Not a bug, not worth chasing |
...
Note: the Troubleshooting section below also documents a *different*
`VIDIOC_S_FMT: Device or resource busy` error, seen in
`journalctl --user -u wireplumber` when a portal app like Cheese fails to
stream. That one isn't the guvcview mmap conflict above — it's a stale,
already-streaming PipeWire node handle left over from boot, fixed by
destroying and recreating the node (see Troubleshooting for the exact
commands).

---

## Appendix — Repo file → system path map

Every configuration file that lives in this repo is a mirror of a file that
must exist at a specific path on the live system for the pipeline to work.
This table is the canonical map between the two — useful for setting a fresh
machine up from scratch, or for spotting drift if `/etc` and this repo ever
disagree (compare with e.g. `diff intel-ipu.conf /etc/v4l2-relayd.d/intel-ipu.conf`).

| Repo file | Destination path | Introduced in | Purpose |
|---|---|---|---|
| [`99-camera.rules`](99-camera.rules) | `/etc/udev/rules.d/99-camera.rules` | Step 2 | Device permissions (`video`/`root` groups) + explicit i2c bind workaround |
| [`72-camera-hide-ipu6-isys.rules`](72-camera-hide-ipu6-isys.rules) | `/etc/udev/rules.d/72-camera-hide-ipu6-isys.rules` | Step 8 | Locks the raw ISYS nodes (`/dev/video0`–`47`) to root so apps don't enumerate them |
| [`intel-ipu.conf`](intel-ipu.conf) | `/etc/v4l2-relayd.d/intel-ipu.conf` | Step 6.1 | Per-instance pipeline config: `VIDEOSRC`, `FORMAT`, `WIDTH`, `HEIGHT`, `FRAMERATE` |
| [`v4l2-relayd`](v4l2-relayd) | `/etc/default/v4l2-relayd` | Step 6.1 | Same variables, loaded first by the unit — `intel-ipu.conf` loads second and overrides it |
| [`icamerasrc.conf`](icamerasrc.conf) | `/etc/systemd/system/v4l2-relayd@intel-ipu.service.d/icamerasrc.conf` | Step 6.2 | systemd drop-in: relaxes `PrivateIPC`/`CapabilityBoundingSet` for CamHAL shared memory |
| [`fixed-binary.conf`](fixed-binary.conf) | `/etc/systemd/system/v4l2-relayd@intel-ipu.service.d/fixed-binary.conf` | Step 6.2b | systemd drop-in: points `ExecStart` at the locally-built `v4l2-relayd` binary |
| [`50-hide-ipu6-isys.conf`](50-hide-ipu6-isys.conf) | `/etc/wireplumber/wireplumber.conf.d/50-hide-ipu6-isys.conf` | Step 8 | WirePlumber: disables `monitor.libcamera`, hides ISYS nodes, tags the virtual cam as "Built-in Webcam" |
| [`modprobe.d-v4l2loopback.conf`](modprobe.d-v4l2loopback.conf)¹ | `/etc/modprobe.d/v4l2loopback.conf` | Step 8 | `v4l2loopback` module options: `video_nr=48`, `card_label`, `exclusive_caps=1` |
| [`ipu6-camera.conf`](ipu6-camera.conf) | `/etc/modprobe.d/ipu6-camera.conf` | Module load order (after Step 1) | `softdep` directives fixing the sensor/bridge/loopback module load order |
| [`intel-camera.conf`](intel-camera.conf) | `/etc/tmpfiles.d/intel-camera.conf` | Module load order (after Step 1) | Creates `/run/camera` at boot for HAL runtime data (currently logs a harmless group-resolution error, see Troubleshooting) |
| [`camera.conf`](camera.conf) | `/etc/modules-load.d/camera.conf` | Module load order (after Step 1) | Loads the sensor + bridge chain: `intel_skl_int3472_discrete`, `intel_cvs`, `ov02c10` |
| [`intel-ipu6.conf`](intel-ipu6.conf) | `/etc/modules-load.d/intel-ipu6.conf` | Module load order (after Step 1) | Loads `intel_ipu6`, `intel_ipu6_isys` |
| [`modules-load.d-v4l2loopback.conf`](modules-load.d-v4l2loopback.conf)¹ | `/etc/modules-load.d/v4l2loopback.conf` | Module load order (after Step 1) | Loads the `v4l2loopback` kernel module |

¹ `modprobe.d-v4l2loopback.conf` and `modules-load.d-v4l2loopback.conf` are
both named `v4l2loopback.conf` on the live system, in two different
directories with different content — renamed here with their parent
directory as a prefix so they can coexist in this flat repo layout.

Not in this table: the helper scripts (`run-webcam.sh`, `camera-check.sh`,
`camera-on-test.sh`, `fix-stuck-webcam-node.sh`) and `pkglist.txt` — these
don't have a fixed system destination; they're meant to be run directly from
this repo.
