...
#
# See README.md, "Troubleshooting: Stuck PipeWire v4l2 Node" for the full
...
kill_stale_holders() {
    ...
    local holder_pids
    holder_pids="$(lsof -t "$DEVICE" 2>/dev/null | sort -u)"
...
udevadm trigger --action=remove "$SYSFS_PATH"
sleep 1
udevadm trigger --action=add "$SYSFS_PATH"
