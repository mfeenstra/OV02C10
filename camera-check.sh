...
modinfo intel_skl_int3472_discrete | grep filename

echo ""
echo "=== 2. Kernel messages (camera probe) ==="
sudo dmesg | grep -iE 'ipu6|ovti|ov02'
...
if dmesg | grep -q "ov02c10.*EPROBE_DEFER\|probe.*deferred"; then
    ...
    echo "STATUS: FAILED"
    dmesg | grep -E "ov02c10.*fail|fail.*ov02c10"
else
    echo "STATUS: (check dmesg above)"
fi
...
media-ctl -p 2>/dev/null | grep -A3 "ov02c10\|OVTI02C1" || echo "(media-ctl not available)"
